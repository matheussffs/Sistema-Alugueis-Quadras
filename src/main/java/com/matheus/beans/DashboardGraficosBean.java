...
public class DashboardGraficosBean implements Serializable {
    ...
    private void createFaturamentoModel() {
        ...
        Query q = em.createNativeQuery(
            "SELECT MONTHNAME(res_dt_inicio) as mes, SUM(res_valor_total) as total " +
            "FROM reservas " +
            "WHERE res_status = 'CONFIRMADA' AND YEAR(res_dt_inicio) = YEAR(CURRENT_DATE()) " +
            "GROUP BY MONTH(res_dt_inicio), mes " +
            "ORDER BY MONTH(res_dt_inicio)");
    ...
    private void createQuadrasModel() {
        ...
        Query q = em.createQuery(
            "SELECT q.quaNome, COUNT(r.resId) as total " +
            "FROM Reserva r JOIN r.quadra q " +
    ...
    private void createModalidadesModel() {
        ...
        Query q = em.createQuery(
            "SELECT m.modNome, COUNT(r.resId) as total " +
            "FROM Reserva r JOIN r.quadra q JOIN q.modalidade m " +
            "GROUP BY m.modNome");
    ...
    private void createStatusModel() {
        ...
        Query q = em.createQuery(
            "SELECT r.resStatus, COUNT(r.resId) as total " +
            "FROM Reserva r GROUP BY r.resStatus");
    ...
    private void createClienteGrowthModel() {
        ...
        Query q = em.createNativeQuery(
            "SELECT MONTHNAME(user_data_cadastro) as mes, COUNT(user_id) as total " +
            "FROM usuarios " +
            "WHERE user_perf_id = (SELECT perf_id FROM perfis WHERE perf_nome = 'CLIENTE') " +
            "AND YEAR(user_data_cadastro) = YEAR(CURRENT_DATE()) " +
            "GROUP BY MONTH(user_data_cadastro), mes " +
    ...
    private void createPeakHoursModel() {
        ...
        Query q = em.createNativeQuery(
            "SELECT HOUR(res_dt_inicio) as hora, COUNT(res_id) as total " +
            "FROM reservas " +
